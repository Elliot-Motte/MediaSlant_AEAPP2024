...
set more off
	...
	foreach var in enviro_vs_jobs /*
			...
			eststo: reghdfe `var' z_RTGFXNC z_RTGMSNBC  $resp_controls,  absorb(state##year)  cluster(fips)
	...
	estfe *, labels(fips "County FEs" year "Year FEs")   
		...
		esttab  using "$replicationdir/03_output/Table_1.tex", r2 label replace se star(* 0.1 ** 0.05 *** 0.01) booktabs  nonotes b(4) se(3) ///	
				drop(_cons) ///
